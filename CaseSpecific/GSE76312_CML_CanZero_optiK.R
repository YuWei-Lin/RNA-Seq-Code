...
# GSE76312_CML Data Preparation
meldat <- read.table("D:/SC Cases Completed/GSE76312_CML_DATA/Cancerous/GSE76312_CMLCan_RAW_CDF_DEL.csv", sep = ",", header = T, stringsAsFactors = F)
Anno <- read.table("D:/SC Cases Completed/GSE76312_CML_DATA/Cancerous/GSE76312_CMLCan_AnnoRows.csv", sep = ",", header = T, stringsAsFactors = F)
colnames(Anno)[1] <- "Gene"
meldat <- rbind(Anno, meldat)
...
tumors_colors=rep(colors[1], 1031)
...
Cell_type <- rep(colorss[1], 1031)
for(i in 2:length(table(as.character(meldat[2,])))){
  Cell_type[meldat[2, ]==names(table(as.character(meldat[2,])))[i]]=colorss[i]
}
...
main_title="GSE76312_CML_Can_tumors_optimalK"
title=paste("D:/SC Cases Completed/GSE76312_CML_DATA/Cancerous/K.Means/Filled with Zero/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 2:12){
  ...
  main_title=paste("GSE76312_CML_CCP_k=", k, sep = "")
  par(cex.main=0.5)
  CCP_colors <- resultss[[k]]$clrs[[3]]
  plot(1:k, col=CCP_colors, pch=16, cex=6)
  Cluster_Colors=rep(CCP_colors[1], 6696)
  for(j in 2:k){
  ...
  tiff(paste("D:/SC Cases Completed/GSE76312_CML_DATA/Cancerous/K.Means/Filled with Zero/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=c(names(table(as.numeric(meldat[1,])))), fill=c(colors[1:20]), border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:as.numeric(k)){
    ...
    write.table(CGset, file = paste("D:/SC Cases Completed/GSE76312_CML_DATA/Cancerous/K.Means/Filled with Zero/ClusterGeneLists/GSE76312_CML_CanClusGenes_K", k, "C", y, ".csv", sep = ""), quote = F, row.names = F, sep = ",") 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/SC Cases Completed/GSE76312_CML_DATA/Cancerous/K.Means/Filled with Zero/GeneColorsLists/GSE76312_CML_Can_CPP_K=", i, ".csv",sep = "")
