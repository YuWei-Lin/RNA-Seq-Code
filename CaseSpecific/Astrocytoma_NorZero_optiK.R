...
title=paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Zero/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 2:12){
  ...
  tiff(paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Zero/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=c("Tcell", "Macro", "oligodendrocyte", "", names(table(as.numeric(meldat[1,])))), fill=c(colorss[1:3], "white", colors[1:length(names(table(as.character(meldat[1,]))))]), border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:k){
    k <- as.numeric(k)
    y <- as.numeric(y)
    ...
    write.table(CGset, file = paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Zero/ClusterGeneLists/Astrocytoma_ClusGenes_K", k, "C", y, ".csv", sep = ""), quote = F, row.names = F, sep = ",") 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Zero/GeneColorsLists/Astrocytoma_CPP_K=", i, ".csv",sep = "")
