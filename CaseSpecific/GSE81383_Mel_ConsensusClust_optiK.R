...
# Mel Data Preparation
meldat <- read.csv("D:/ALL_FINAL_RESULTS/GSE81383_Melanoma_DATA/K.Means Clustering/Inputs/GSE81383_Melanoma_RAW_CDF.csv", header = TRUE, stringsAsFactors = FALSE)
...
tumors_colors=rep(colors[1], 307)
...
title=paste("D:/ALL_FINAL_RESULTS/GSE81383_Melanoma_DATA/K.Means Clustering/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 2:12){
  ...
  main_title=paste("GSE81383_Melanoma_CCP_k=", k, sep = "")
  par(cex.main=0.5)
  CCP_colors <- resultss[[k]]$clrs[[3]]
  plot(1:k, col=CCP_colors, pch=16, cex=6)
  Cluster_Colors=rep(CCP_colors[1], 8097)
  for(j in 2:k){
  ...
  tiff(paste("D:/ALL_FINAL_RESULTS/GSE81383_Melanoma_DATA/K.Means Clustering/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=c(names(table(as.character(meldat[1,])))), fill=colors[1:3], border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:k){
    CGset <- rownames(CC)[resultss[[k]][["consensusClass"]]==y]
    write.table(CGset, file = paste("D:/ALL_FINAL_RESULTS/GSE81383_Melanoma_DATA/K.Means Clustering/ClusterGeneLists/GSE81383_Melanoma_ClusGenes_K", k, "_C", y, ".txt", sep = ""), quote = F, row.names = F) 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/ALL_FINAL_RESULTS/GSE81383_Melanoma_DATA/K.Means Clustering/GeneColorsLists/GSE81383_Melanoma_CPP_K=", i, ".csv",sep = "")
