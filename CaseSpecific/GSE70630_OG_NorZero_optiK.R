...
# GSE70630_OG Data Preparation
meldat <- read.csv("D:/SC Cases Completed/GSE70630_OG_DATA/Normal/Processed Matrices/GSE70630_OGNor_RAW_CDF.csv", header = TRUE, stringsAsFactors = FALSE)
...
tumors_colors=rep(colors[1], 280)
...
title=paste("D:/SC Cases Completed/GSE70630_OG_DATA/Normal/K.Means/Filled with Zero/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 2:12){
  ...
  Cluster_Colors=rep(CCP_colors[1], 5563)
  for(j in 2:k){
  ...
  tiff(paste("D:/SC Cases Completed/GSE70630_OG_DATA/Normal/K.Means/Filled with Zero/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=c("Macrophage", "Oligodendrocyte", "", names(table(as.numeric(meldat[1,])))), fill=c(colorss[1:2], "white", colors[1:3]), border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:as.numeric(k)){
    ...
    write.table(CGset, file = paste("D:/SC Cases Completed/GSE70630_OG_DATA/Normal/K.Means/Filled with Zero/ClusterGeneLists/GSE70630_OG_ClusGenes_K", k, "C", y, ".csv", sep = ""), quote = F, row.names = F, sep = ",") 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/SC Cases Completed/GSE70630_OG_DATA/Normal/K.Means/Filled with Zero/GeneColorsLists/GSE81383_Melanoma_CPP_K=", i, ".csv",sep = "")
