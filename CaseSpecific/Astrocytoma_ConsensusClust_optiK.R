...
meldat <- read.csv("D:/ALL_FINAL_RESULTS/Astrocytoma_DATA/K.Means Clustering/Inputs/AstrocytomaCan_RAW_CDF.csv", header = TRUE, stringsAsFactors = FALSE)
...
labels <- strsplit(colnames(meldat), "_")
labels <- unlist(lapply(labels, function(x) x[1]))
tumors_colors=rep(colors[1], 4644)
...
title=paste("D:/ALL_FINAL_RESULTS/Astrocytoma_DATA/K.Means Clustering_UsingMean/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 4:12){
  ...
  Cluster_Colors=rep(CCP_colors[1], 8816)
  for(j in 2:k){
  ...
  tiff(paste("D:/ALL_FINAL_RESULTS/Astrocytoma_DATA/K.Means Clustering_UsingMean/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=c(names(table(as.numeric(meldat[1,])))), fill=colors[1:9], border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:k){
    k <- as.numeric(k)
    y <- as.numeric(y)
    ...
    write.table(CGset, file = paste("D:/ALL_FINAL_RESULTS/Astrocytoma_DATA/K.Means Clustering/ClusterGeneLists/Astrocytoma_ClusGenes_K", k, "C", y, ".csv", sep = ""), quote = F, row.names = F, sep = ",") 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/ALL_FINAL_RESULTS/Astrocytoma_DATA/K.Means Clustering/GeneColorsLists/Astrocytoma_CPP_K=", i, ".csv",sep = "")
...
tiff(paste("D:/ALL_FINAL_RESULTS/Astrocytoma_DATA/K.Means Clustering/Astrocytoma_Mean.tiff", sep=""), width=1600, height=1600, compression="lzw", res=300)
plot(tsne$Y, main= "Astrocytoma_UsingMean", cex.main = 0.8, col= tumors_colors, pch = 16, cex = 0.4)
legend("topright",legend=c(names(table(as.numeric(meldat[1,])))), fill=colors[1:9], border=FALSE, bty="n", y.intersp = 1, cex=0.7)
dev.off()