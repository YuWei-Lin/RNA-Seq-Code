...
# GSE72056 Data Preparation
meldat <- read.table("D:/SC Cases Completed/GSE72056_Melanoma_DATA/Normal/Processed Matrices/GSE72056_MelanomaNor_RAW_CDF.csv", sep = ",", header = T, stringsAsFactors = F)
...
meldat <- meldat[ , !(meldat[3, ] == 0)]
meldat <- meldat[apply(meldat, 1, function(x) sum(is.na(x)) < (ncol(meldat)*(0.7))), ]
meldat <- meldat[ ,order(meldat[3,], meldat[1,])] #Tumor ID arrange columns according to multiple row values
...
tumors_colors=rep(colors[1], 2772)
...
Cell_type <- rep(colorss[1], 2772)
for(i in 2:length(table(as.character(meldat[3,])))){
  Cell_type[meldat[3, ]==names(table(as.character(meldat[3,])))[i]]=colorss[i]
}
...
main_title="GSE72056_Melanoma_Nor_tumors_optimalK"
title=paste("D:/SC Cases Completed/GSE72056_Melanoma_DATA/Normal/K.Means/Filled with Mean/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 2:12){
  ...
  main_title=paste("GSE72056_Melanoma_CCP_k=", k, sep = "")
  par(cex.main=0.5)
  CCP_colors <- resultss[[k]]$clrs[[3]]
  plot(1:k, col=CCP_colors, pch=16, cex=6)
  Cluster_Colors=rep(CCP_colors[1], 4741)
  for(j in 2:k){
  ...
  tiff(paste("D:/SC Cases Completed/GSE72056_Melanoma_DATA/Normal/K.Means/Filled with Mean/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=c("T", "B", "Macro", "Endo", "CAF", "NK", "", names(table(as.numeric(meldat[1,])))), fill=c(colorss, "white", colors[1:19]), border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:as.numeric(k)){
    ...
    write.table(CGset, file = paste("D:/SC Cases Completed/GSE72056_Melanoma_DATA/Normal/K.Means/Filled with Mean/ClusterGeneLists/GSE72056_Mel_NorClusGenes_K", k, "C", y, ".csv", sep = ""), quote = F, row.names = F, sep = ",") 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/SC Cases Completed/GSE72056_Melanoma_DATA/Normal/K.Means/Filled with Mean/GeneColorsLists/GSE72056_Melanoma_Nor_CPP_K=", i, ".csv",sep = "")
