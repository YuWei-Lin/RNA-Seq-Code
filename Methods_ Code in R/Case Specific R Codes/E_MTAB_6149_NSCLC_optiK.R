...
### E_MTAB_6149_NSCLC Data Preparation
meldat <- read.csv("D:/SC Cases Completed/E_MTAB_6149_NSCLC_DATA/Cancerous/Processed Matrices/E_MTAB_6149_NSCLC_CDFwNA.csv", header = TRUE, stringsAsFactors = F)
sampleinfo <- read.csv("D:/SC Cases Completed/E_MTAB_6149_NSCLC_DATA/Cancerous/Processed Matrices/E_MTAB_6149_SampleInfo.csv", header = TRUE, stringsAsFactors = F)
meldat <- meldat[!duplicated(meldat[ , 1]), ]
rownames(meldat) <- meldat$Gene
meldat <- meldat[,-1]
meldat <- rbind(as.numeric(sub("patient_", "", sampleinfo$PatientID)), meldat)
rownames(meldat)[1] <- "Patient_ID"
...
tumors_colors=rep(colors[1], 3524)
...
main_title="E_MTAB_6149_NSCLC_tumors_optimalK"
title=paste("D:/SC Cases Completed/E_MTAB_6149_NSCLC_DATA/Cancerous/K.Means/Filled with Mean/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 2:12){
  ...
  main_title=paste("E_MTAB_6149_NSCLC_CCP_k=", k, sep = "")
  par(cex.main=0.5)
  CCP_colors <- resultss[[k]]$clrs[[3]]
  plot(1:k, col=CCP_colors, pch=16, cex=6)
  Cluster_Colors=rep(CCP_colors[1], 3455)
  for(j in 2:k){
  ...
  tiff(paste("D:/SC Cases Completed/E_MTAB_6149_NSCLC_DATA/Cancerous/K.Means/Filled with Mean/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=names(table(as.character(meldat[1,]))), fill=c(colors[1:length(names(table(as.character(meldat[1,]))))]), border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:as.numeric(k)){
    ...
    write.table(CGset, file = paste("D:/SC Cases Completed/E_MTAB_6149_NSCLC_DATA/Cancerous/K.Means/Filled with Mean/ClusterGeneLists/E_MTAB_6149_NSCLC_ClusGenes_K", k, "C", y, ".csv", sep = ""), quote = F, row.names = F, sep = ",") 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/SC Cases Completed/E_MTAB_6149_NSCLC_DATA/Cancerous/K.Means/Filled with Mean/GeneColorsLists/E_MTAB_6149_NSCLC_CPP_K=", i, ".csv",sep = "")
  write.csv(dfg1, file1, row.names = FALSE)
}
### The genes in K01C01 are all #85848f color.
