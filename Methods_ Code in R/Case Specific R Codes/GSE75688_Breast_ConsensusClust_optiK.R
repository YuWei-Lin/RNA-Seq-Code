...
meldat <- read.csv("D:/ALL_R_scripts_YUWEI/GSE75688_Breast_tumors_optimalK/GSE75688_Breast_NoBulk_CDF.csv", header = TRUE, stringsAsFactors = FALSE)
...
tumors_colors=rep(colors[1], ncol(meldat))
...
CC <- meldat # "CC" a duplicate
### TCGA Methods: ConsensusClusterPlus
# try http:// if https:// URLs are not supported
# source("https://bioconductor.org/biocLite.R")
# biocLite("ConsensusClusterPlus")
library(ConsensusClusterPlus)
title=paste("D:/GSE75688_Breast_CCP_kClusters/BC_Version2", sep = "")
...
for(k in 2:12){
  ...
  Cluster_Colors=rep(CCP_colors[1], 8318)
  for(j in 2:k){
  ...
  tiff(paste("D:/GSE75688_Breast_CCP_kClusters/BC_Version2/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  for(y in 1:k){
    ...
    write.table(CGset, file = paste("D:/GSE75688_Breast_CCP_kClusters/BC_Version2/GSE75688_Breast_ClusGenes_K", k, "_C", y, ".txt", sep = ""), quote = F, row.names = F) 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("GSE75688_Breast_CPP_K=", i, ".csv",sep = "")
