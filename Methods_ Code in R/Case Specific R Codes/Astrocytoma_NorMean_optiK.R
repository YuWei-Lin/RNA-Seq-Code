...
# Astrocyte Data Preparation
meldat <- read.csv("D:/SC Cases Completed/Astrocytoma_DATA/Normal/Processed Matrices/AstrocytomaNor_RAW_CDF.csv", header = TRUE, stringsAsFactors = FALSE)
meldat <- meldat[!duplicated(meldat[ , 1]), ]
rownames(meldat) <- meldat$X
meldat <- meldat[ ,-1] 
meldat <- meldat[ , !(grepl( "Bulk" , names( meldat ))|grepl( "Prim" , names( meldat ) ))]
# Epithelial=1 ; Tcell=2 ; Bcell=3 ; Macrophage=4 ; Fibroblast=5
sampleinfo$CellType <- sub("Epithelial", 1, sampleinfo$CellType)
sampleinfo$CellType <- sub("Tcell", 2, sampleinfo$CellType)
sampleinfo$CellType <- sub("Bcell", 3, sampleinfo$CellType)
sampleinfo$CellType <- sub("Macrophage", 4, sampleinfo$CellType)
sampleinfo$CellType <- sub("Fibroblast", 5, sampleinfo$CellType)
...
tumors_colors=rep(colors[1], 1289)
...
colorss = c("#000075", "#f58231", "#e6beff", "#42d4f4", "#469990", "#800000", "#aaffc3", "#ffe119")
Cell_type <- rep(colorss[1], 1289)
...
title=paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Mean/ConsensusClusterPlus.Figures", sep = "")
...
for(k in 2:12){
  ...
  main_title=paste("Astrocytoma_CCP_k=", k, sep = "")
  par(cex.main=0.5)
  CCP_colors <- resultss[[k]]$clrs[[3]]
  plot(1:k, col=CCP_colors, pch=16, cex=6)
  Cluster_Colors=rep(CCP_colors[1], 6185)
  for(j in 2:k){
  ...
  tiff(paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Mean/K_Heatmaps/", main_title, ".tiff", sep=""), width=2200, height=1600, compression="lzw", res=300)
  ...
  legend("topright",legend=c("Tcell", "Macro", "oligodendrocyte", "", names(table(as.numeric(meldat[1,])))), fill=c(colorss[1:3], "white", colors[1:length(names(table(as.numeric(meldat[1,]))))]), border=FALSE, bty="n", y.intersp = 1, cex=0.7)
  dev.off()
  for(y in 1:k){
    k <- as.numeric(k)
    y <- as.numeric(y)
    ...
    write.table(CGset, file = paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Mean/ClusterGeneLists/Astrocytoma_ClusGenes_K", k, "C", y, ".csv", sep = ""), quote = F, row.names = F, sep = ",") 
  }
}
t_time <- proc.time()-p_time
print(t_time)
...
for(i in 2:12){
  ...
  file1 <- paste("D:/SC Cases Completed/Astrocytoma_DATA/Normal/K.Means/Filled with Mean/GeneColorsLists/Astrocytoma_CPP_K=", i, ".csv",sep = "")
