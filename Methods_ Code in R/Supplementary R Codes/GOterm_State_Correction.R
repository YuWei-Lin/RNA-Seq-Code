...
GOdex.Z <- read.csv(paste(Filepath, "_GOindex_Z.csv", sep = ""), header = F, stringsAsFactors = F)
GOstate.Z <- read.csv(paste(Filepath, "_state_Z.csv", sep = ""), header = F, stringsAsFactors = F)
GOFDRs.Z <- read.csv(paste(Filepath, "_clusFDRs_Z.csv", sep = ""), header = F, stringsAsFactors = F)

GOterms <- GOterms$V1
GOdex.Z <- as.numeric(GOdex.Z)
GOFDRs.Z <- GOFDRs.Z[GOdex.Z, ]
### Process states with Zero replacement
...
for (i in 1:length(GOdex.Z)) {
  GOname <- GOterms[as.numeric(GOdex.Z[i])]
  Chefile <- rbind(colnames(BVC)[2:ncol(BVC)], 1:78, round(-log10(GOFDRs.Z[i, ]), digits = 2), GOstate.Z[i, ], GOstate.Z[i, ])
  Chefile <- t(Chefile)
  colnames(Chefile) <- c("Cluster", "ClustNumber", "neglog10FDRs", "Predicted State", "Modified State")
  write.table(GOname, file = paste("D:/ALL_FINAL_RESULTS/Manual_Check_States/", CaseN, "/", CaseN, "_ZeroRep/", GOname, "_ZeroRep_stateChange.csv", sep = ""), sep = ",", row.names = F, col.names = F)
  write.table(Chefile, file = paste("D:/ALL_FINAL_RESULTS/Manual_Check_States/", CaseN, "/", CaseN, "_ZeroRep/", GOname, "_ZeroRep_stateChange.csv", sep = ""), sep = ",", row.names = F, col.names = T, append = T)  
}
...
destFolder <- paste("D:/ALL_FINAL_RESULTS/Manual_Check_States/", CaseN, "/", CaseN, "_ZeroRep", sep = "")
